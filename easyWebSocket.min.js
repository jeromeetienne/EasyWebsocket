...
null}this._sendBuffer=[];io.Transport.prototype._onDisconnect.call(this)};e.prototype._request=function(g,l,q){var o=c(this.base._isXDomain());if(q)o.multipart=true;o.open(l||"GET",this._prepareUrl()+(g?"/"+g:""));l=="POST"&&"setRequestHeader"in o&&o.setRequestHeader("Content-type","application/x-www-form-urlencoded; charset=utf-8");return o};e.check=function(g){try{if(c(g))return true}catch(l){}return false};e.xdomainCheck=function(){return e.check(true)};e.request=c})();
...
JSONPPolling.prototype._send=function(a){function b(){c();e._posting=false;e._checkSend()}function c(){e._iframe&&e._form.removeChild(e._iframe);try{o=document.createElement('<iframe name="'+e._iframeId+'">')}catch(z){o=document.createElement("iframe");o.name=e._iframeId}o.id=e._iframeId;e._form.appendChild(o);e._iframe=o}var e=this;if(!("_form"in this)){var g=document.createElement("FORM"),l=document.createElement("TEXTAREA"),q=this._iframeId="socket_io_iframe_"+this._index,o;g.style.position="absolute";
...
function(){this.connectTimeoutTimer&&clearTimeout(this.connectTimeoutTimer);this.transport.disconnect();return this};a.prototype.on=function(b,c){b in this._events||(this._events[b]=[]);this._events[b].push(c);return this};a.prototype.emit=function(b,c){if(b in this._events)for(var e=this._events[b].concat(),g=0,l=e.length;g<l;g++)e[g].apply(this,c===undefined?[]:c);return this};a.prototype.removeEvent=function(b,c){if(b in this._events)for(var e=0,g=this._events[b].length;e<g;e++)this._events[b][e]==
...
a,false);if(k.ie&&k.win){m.attachEvent("onreadystatechange",function(){if(m.readyState=="complete"){m.detachEvent("onreadystatechange",arguments.callee);a()}});u==top&&function(){if(!E){try{m.documentElement.doScroll("left")}catch(d){setTimeout(arguments.callee,0);return}a()}}()}k.wk&&function(){E||(/loaded|complete/.test(m.readyState)?a():setTimeout(arguments.callee,0))}();c(a)}}})();(function(){k.ie&&k.win&&window.attachEvent("onunload",function(){for(var d=G.length,f=0;f<d;f++)G[f][0].detachEvent(G[f][1],
G[f][2]);d=P.length;for(f=0;f<d;f++)X(P[f]);for(var h in k)k[h]=null;k=null;for(var i in swfobject)swfobject[i]=null;swfobject=null})})();return{registerObject:function(d,f,h,i){if(k.w3&&d&&f){var j={};j.id=d;j.swfVersion=f;j.expressInstall=h;j.callbackFn=i;A[A.length]=j;F(d,false)}else i&&i({success:false,id:d})},getObjectById:function(d){if(k.w3)return l(d)},embedSWF:function(d,f,h,i,j,p,n,s,t,J){var Q={success:false,id:f};if(k.w3&&!(k.wk&&k.wk<312)&&d&&f&&h&&i&&j){F(f,false);b(function(){h+="";
...
function FABridge__bridgeInitialized(a){var b=document.getElementsByTagName("object"),c=b.length,e=[];if(c>0)for(var g=0;g<c;g++)if(typeof b[g].SetVariable!="undefined")e[e.length]=b[g];c=document.getElementsByTagName("embed");g=c.length;b=[];if(g>0)for(var l=0;l<g;l++)if(typeof c[l].SetVariable!="undefined")b[b.length]=c[l];l=e.length;c=b.length;g="bridgeName="+a;if(l==1&&!c||l==1&&c==1)FABridge.attachBridge(e[0],a);else if(c==1&&!l)FABridge.attachBridge(b[0],a);else{var q=false;if(l>1)for(var o=
